...
| **STP (802.1D)** | Spanning Tree Protocol — prevents L2 loops by blocking redundant path |
...
End-to-end delivery of data across interconnected networks.

### Subnetting and CIDR notation
Splitting a network into smaller networks (subnets)
- Network ID → first IP in subnet
- Broadcast ID → last IP in subnet
- Total IPs = 2^n, Usable IPs = 2^n - 2; where n = number of host bits
- CIDR notation: IP/Prefix ex: 192.168.1.0/24 ;first 24(Prefix) bits = network, remaining bits = hosts
- Subnet mask written like: 255.255.255.0(11111111.11111111.11111111.00000000) shows network bits=1s, Host bits=0s

### Steps to solve problems
- Identify CIDR /x 
- Find subnet mask
- Calculate block size(Block size = 256 - subnet mask octet)...see example below.
- List subnet ranges
- Find: Network ID, First IP, Last IP, Broadcast

Subnets increase by block size
...
| **OSPF** | Open Shortest Path First (link-state IGP), best path for routing |
| **EIGRP** | Cisco's Enhanced Interior Gateway Routing Protocol |
| **BGP** | Border Gateway Protocol (internet's backbone routing), exhange routing info between autonomous systems |
...
### Layman's Terms
This layer is the **shipping department** of networking. It decides whether to send your package via **express guaranteed delivery (TCP)** or **cheap untracked shipping (UDP)**. It also labels which "window" at the post office (port number) the package is headed to. Port is a logical endpoint on host that applications use to communicate.

### Formal Definition
The Transport Layer provides end-to-end communication services for applications. It offers reliable (TCP) or best-effort (UDP) delivery, multiplexing via port numbers, flow control, congestion control, and (with TCP) connection establishment/termination via a three-way handshake, data segmentation and error correction.

### TCP vs UDP — Deep Comparison

```
TCP (Transmission Control Protocol)(Segments):         UDP (User Datagram Protocol)(Datagrams):
────────────────────────────────────         ────────────────────────────
...
### Formal Definition
The Session Layer establishes, manages, and terminates sessions (logical connections) between communicating systems. It provides services for dialog control (half-duplex vs full-duplex), synchronization (checkpointing for recovery), and session multiplexing. Data unit: Data

### Key Responsibilities
- Session establishment, maintenance, termination
- Dialog control (who speaks when)
- Synchronization points (checkpoints for recovery after temporary suspension of sessions)
...
### Formal Definition
The Presentation Layer is responsible for the formatting, translation, encryption/decryption(SSL/TLS handshake), and compression of data. It ensures that data from the Application Layer of one system can be read by the Application Layer of another by translating between different data formats and character encodings. Data unit: Data
...
### Formal Definition
The Application Layer provides network services directly to end-user applications. It is the highest layer of the OSI model and interfaces directly with software applications to provide communication functions as needed. It is not the application itself, but the protocols and services applications use to communicate.Data unit: Data
...
```bash
# L1: Is the cable/interface up? check cables, connectors, hardware
ip link show eth0
ethtool eth0

# L2: Are we getting ARP responses? verify switch configurations and VLAN assignments
arping 192.168.1.1
arp -a

# L3: Can we route (ping gateway)? check route tables, ipconfigs and subnet masks
ping 192.168.1.1
traceroute 8.8.8.8

# L4: Is the service port open? analyze ports and packet flow
nc -zv 192.168.1.100 443
nmap -p 443 192.168.1.100

# L5-L7: Is the application responding? check session protocols, verify SSL/TLS configs and keys, inspect DNS and application service status
